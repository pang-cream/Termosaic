...
def build_en_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  uv run termosaic Data/cat.mp4 --kind video --speed 2 --scale 1.3 --width 160\n"
...
def add_common_arguments(parser: argparse.ArgumentParser, *, language: str) -> None:
    if language == "zh":
        ...
        parser.add_argument("--width", type=positive_int, help="输出宽度，单位是终端列；不传则自动适配终端")
        parser.add_argument("--height", type=positive_int, help="输出高度，单位是终端行；half 模式会采样两倍图像行数")
    ...
    parser.add_argument("--width", type=positive_int, help="output width in terminal columns; auto-fits when omitted")
    parser.add_argument("--height", type=positive_int, help="output height in terminal rows; half mode samples twice as many image rows")
    ...
    try:
        with Image.open(path) as image:
            if getattr(image, "is_animated", False) and getattr(image, "n_frames", 1) > 1:
                frames, durations = prepare_animation(image, args=args, background=background)
